...
IMGS_DIR = f'dcganc_distr_images{RES}'
...
def make_cdiscriminator_model128(cond_dim):
  ...
  #attrs = tf.broadcast_to(attrs, shape=(96, 128, 6))
  c_input = tf.concat([image, attrs], axis=-1)
  output = model(c_input)
...
@tf.function
def distributed_train_step(images, attrs):
  per_replica_gen_loss, per_replica_disc_loss = strategy.run(train_step, args=(images, attrs))
...
def train(dataset, epochs):
  ...
  while epoch < EPOCHS:
    ...
    for image_batch in dataset:
      ...
      g_loss, d_loss = distributed_train_step(images, attrs)
      if i==1000:
        break
