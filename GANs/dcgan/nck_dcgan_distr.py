...
#---
def res_decor(res=RES):
  def ext_decor(_extract_fn):
    def _extract_fn_dec(tfrecord):
      return _extract_fn(tfrecord, res)
    return _extract_fn_dec
  return ext_decor
...
def res_extract_fn(tfrecord):
  part_func = functools.partial(_extract_fn, res=RES)
  return part_func(tfrecord)
...
#dataset = dataset.map(res_extract_fn)
#dataset = dataset.map(lambda x: _extract_fn(x, res=RES))
#dataset = dataset.map(functools.partial(_extract_fn, res=RES))
